...
doc_dir = os.path.join(base_dir, "..", "..", "documentation", "Aufgabe_1", "transfer_learning", "vgg16", f"training_results_{current_date}")
...
def get_transfer_model(num_classes, device): #resnet18
    ...
    model.classifier[6] = nn.Linear(model.classifier[6].in_features, num_classes)  # Change the last layer to match num_classes
    model.classifier[6].requires_grad = True  # Allow training of the last layer
...
def train_model(model, train_loader, val_loader, device, patience, dataset):
    ...
    early_stopper = EarlyStopper(patience=patience, delta=0.05)
